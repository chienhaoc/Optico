...
* **Spatially Adaptive LR Pre-emphasis (Phase 8.0)**: On JPEG inputs, applies a spatially adaptive high-pass filter (Sobel-mask modulated, max $\alpha = 0.55$) to LR images prior to warp. This restricts pre-emphasis sharpening to true high-frequency edges, preventing flat-area noise amplification while reducing post-deconvolution ringing.
